...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "MTseekerData", "tests")); system.time(test_check("MTseekerData", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran MTseekerData_$(VERSION).tar.gz
	rm -rf MTseekerData.Rcheck/
